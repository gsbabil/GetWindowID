...
int main(int argc, char **argv) {
  if (argc != 3) {
    fprintf(stderr,
            ...
            "\n"
            "To list all running apps, pass `--all` as app name:\n"
            "    %s --all --list\n"
            "\n"
            "To get the ID of a window without a title, pass an empty string "
            "as the title:\n"
            "    %s GLFW ''\n"
            "\n"
  ...
  bool showAllApp = [requestedApp isEqualToString:@"--all"];
  ...
  for (NSDictionary *window in windows) {
    ...
    CGRectMakeWithDictionaryRepresentation(
        (CFDictionaryRef)window[(NSString *)kCGWindowBounds], &currentBounds);

    if (showAllApp ||
        [[currentApp lowercaseString]
            isEqualToString:[requestedApp lowercaseString]] ||
        [[currentApp lowercaseString]
            containsString:[requestedApp lowercaseString]]) {
      appFound = true;

      double aspect = currentBounds.size.width / currentBounds.size.height;
      if (aspect > 30)
        // If it's that wide and short, it's probably the system menu bar, so
        // ignore it.
        continue;

      // If CGWindowListCopyWindowInfo didn't give us the window title, try to
      // extract it from the Accessibility API.
      if (currentWindowTitle.length == 0) {
        AXUIElementRef axapp = AXUIElementCreateApplication(
            [window[(NSString *)kCGWindowOwnerPID] intValue]);
        NSArray *windows;
        if (AXUIElementCopyAttributeValue(axapp, kAXWindowsAttribute,
                                          (CFTypeRef *)&windows) ==
            kAXErrorSuccess)
          for (id window in windows) {
            // The Accessibility API doesn't expose a unique identifier for the
            // window, so guess based on its frame.
            ...
            if (AXUIElementCopyAttributeValue((AXUIElementRef)window,
              ...
              if (AXValueGetValue(value, kAXValueTypeCGPoint, &p))
                if (p.x != currentBounds.origin.x ||
                    p.y != currentBounds.origin.y)
                  possibleMatch = false;
            }
            if (AXUIElementCopyAttributeValue((AXUIElementRef)window,
                                              kAXSizeAttribute,
              ...
              if (AXValueGetValue(value, kAXValueTypeCGSize, &s))
                if (s.width != currentBounds.size.width ||
                    s.height != currentBounds.size.height)
                  possibleMatch = false;
            }

            if (possibleMatch) {
              AXUIElementCopyAttributeValue((AXUIElementRef)window,
                                            kAXTitleAttribute,
      ...
      if ([currentWindowTitle isEqualToString:@"Focus Proxy"] &&
          currentBounds.size.width == 1 && currentBounds.size.height == 1)
        continue;
      ...
      if ([[currentWindowTitle lowercaseString]
              isEqualToString:[requestedWindow lowercaseString]] ||
          [[currentWindowTitle lowercaseString]
              containsString:[requestedWindow lowercaseString]] ||
